...
@RestController
@RequestMapping("/api/admin/inquiry-templates")
public class AdminInquiryReplyTemplateController {

	private final InquiryReplyTemplateService service;

	public AdminInquiryReplyTemplateController(InquiryReplyTemplateService service) {
	...
	@GetMapping("/{id}")
	public ResponseEntity<InquiryReplyTemplateResponse> getById(@PathVariable Long id) {
		return ResponseEntity.ok(service.findById(id));
	}

	@PostMapping
	public ResponseEntity<InquiryReplyTemplateResponse> create(@Valid @RequestBody InquiryReplyTemplateRequest request) {
